...
When the [approval gate](/docs/configuration#approval) is enabled, high-risk tool calls (`sandbox_exec`, `opencode`, `manage_skill`) trigger an inline keyboard with **Approve** and **Reject** buttons. The agent waits for the user's response before executing. If the user doesn't respond within the timeout, the tool call is auto-rejected. Only users in `allowedUsers` can respond to approval buttons.
